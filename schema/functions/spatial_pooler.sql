 ...
 */
CREATE FUNCTION htm.spatial_pooler_compute(indexes INT[])
RETURNS BOOLEAN
AS $$ 
DECLARE
  iterations CONSTANT INT := htm.spatial_pooler_get('compute_iterations');
BEGIN
  -- Calculate Overlap.
  ...
  SELECT col.id, COUNT(synapse.id) AS overlap
    ...
    JOIN htm.link_input_synapse
      ...
      AND link_input_synapse.input_index IN unnest(indexes)
    GROUP BY col.id
    ORDER BY col.id;  
  */
