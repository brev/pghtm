...
BEGIN
  EXECUTE FORMAT($sql$
    CREATE TABLE htm.config(
      ...
      --  @TemporalMemory
      cell_count INT NOT NULL DEFAULT %1$L,
      CHECK (cell_count = %1$L),  -- CONSTANT

      -- column_count: # of columms per region
      --  @SpatialPooler
      column_count INT NOT NULL DEFAULT %2$L,
      CHECK (column_count = %2$L),  -- CONSTANT

      -- input_width: Input SDR Bit Width
      --  @SpatialPooler
      input_width INT NOT NULL DEFAULT %2$L,
      CHECK (input_width = %2$L),  -- CONSTANT
      ...
      -- synapse_proximal_count: # synapses per column to input
      --  @SpatialPooler
      synapse_proximal_count INT NOT NULL DEFAULT %6$L,
      CHECK (synapse_proximal_count = %6$L),  -- CONSTANT
      ...
      -- column_synapse_threshold: # active synapses required for an
      --  active column. Can be used like a low-pass noise filter.
      --  nupic sp:stimulusThreshold=0
      --  @SpatialPooler
      column_synapse_threshold INT NOT NULL DEFAULT 0,
  ...
  $sql$,
    ...
    -- 5$ synapse_distal_spread_pct
    synapse_distal_spread_pct,

    -- 6$ synapse_proximal_count
